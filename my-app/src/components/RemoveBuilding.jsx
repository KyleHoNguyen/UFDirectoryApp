function RemoveBuilding({data, id, buildingUpdate}){

    // creates deep copy of the data 
    const temp = JSON.parse(JSON.stringify(data));
    // If the button is clicked then the array filters out the arrays wuthout the matching id
    // and then updates the data
    function handleCLick(){
        buildingUpdate(Array.from(temp).filter(building => building.id !== id));
    }
    // Actual remove button
    // Uses building icon from bootstrap
    return (
        <button 
        className="button1"
        onClick={handleCLick}
        >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-building-x" viewBox="0 0 16 16">
